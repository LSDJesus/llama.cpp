...
llm_build_qwen35moe::llm_build_qwen35moe(const llama_model & model, const llm_graph_params & params) :
    ...
    //   hidden_states[-2] = output of the last transformer block (attn/SSM + MoE FFN + residual),
    //                        BEFORE output_norm is applied  ← we capture here
    // NOTE: Qwen3.5 MoE is a hybrid SSM/Transformer+MoE model; recurrent layers contribute to this.
